    ...
    VAR(uint8, AUTOMATIC) CtrlrIdx)
{
    ...
    else
    {
        ...
        HwCanCtrlr[Luc_HwId].CtrlNSts->ulMCReg.NART = REGISTER_BIT_SET;
        #endif
        ...
        HwCanCtrlr[Luc_HwId].CtrlNSts->ulMCReg.TXFP = REGISTER_BIT_SET;
        #endif

        /* setup CAN controller default baudrate */
        HwCanCtrlr[Luc_HwId].CtrlNSts->ulBTReg.val = REGISTER_RESET_VALUE;
        HwCanCtrlr[Luc_HwId].CtrlNSts->ulBTReg.SJW = Lpt_Ctrlr->ptDefaultBaudrateCfg->ucBaudrateSJW;
        HwCanCtrlr[Luc_HwId].CtrlNSts->ulBTReg.TS2 = Lpt_Ctrlr->ptDefaultBaudrateCfg->ucBaudrateSeg2;
        HwCanCtrlr[Luc_HwId].CtrlNSts->ulBTReg.TS1 = Lpt_Ctrlr->ptDefaultBaudrateCfg->ucBaudrateSeg1;
        HwCanCtrlr[Luc_HwId].CtrlNSts->ulBTReg.BRP = Lpt_Ctrlr->ptDefaultBaudrateCfg->usBaudrateBRP;

        /* configure hardware object and hardware filter of CAN controller */
        /* enter initialization mode to config filter */
        HwCanCtrlr[Luc_HwId].Filter->ulFMReg.FINIT = REGISTER_BIT_SET;

        /* define the start bank for the CAN2 interface (Slave) in the range 0 to 27 */
        if (Luc_HwId == CAN_HW_CTRLR1)
        {
            HwCanCtrlr[Luc_HwId].Filter->ulFMReg.CAN2SB = Lpt_Ctrlr->ucFltBankStart;
        }
        else
        {
            /* empty */
    ...
    VAR(uint8, AUTOMATIC) CtrlrIdx)
{
    ...
    else
    {
        ...
        /* clear baudrate */
        HwCanCtrlr[Luc_HwId].CtrlNSts->ulBTReg.val = REGISTER_RESET_VALUE;
        HwCanCtrlr[Luc_HwId].Filter->ulFMReg.val = REGISTER_RESET_VALUE;
        HwCanCtrlr[Luc_HwId].Filter->ulFMReg.FINIT = REGISTER_BIT_SET;
        for (Luc_Count = 0; Luc_Count < CAN_HW_FILTER_BANK_MAX; Luc_Count++)
        {
            HwCanCtrlr[Luc_HwId].Filter->ulFM1Reg = REGISTER_RESET_VALUE;
            HwCanCtrlr[Luc_HwId].Filter->ulFS1Reg = REGISTER_RESET_VALUE;
            HwCanCtrlr[Luc_HwId].Filter->ulFFA1Reg = REGISTER_RESET_VALUE;
            HwCanCtrlr[Luc_HwId].Filter->ulFReg[Luc_Count][FILTER_BANK_ID] = REGISTER_RESET_VALUE;
            HwCanCtrlr[Luc_HwId].Filter->ulFReg[Luc_Count][FILTER_BANK_MASK] = REGISTER_RESET_VALUE;
            HwCanCtrlr[Luc_HwId].Filter->ulFA1Reg = REGISTER_RESET_VALUE;
        }
        HwCanCtrlr[Luc_HwId].Filter->ulFMReg.FINIT = REGISTER_BIT_CLEAR;
 ...
 *  Sync/Async........ : Synchronous
 *  Reentrancy........ : Non Reentrant
 *  Parameters (IN)... : Idx, FltBankStart, FltBankEnd, FltBankType, HwFilterRule
 *  Parameters (INOUT) : None
 *  Parameters (OUT).. : None
 *  Return Value...... : boolean(TRUE/FALSE)
 *  Register usage     :
    ...
    uint8 FltBankType, Can_HwFilterConfigType *HwFilterRule)
{
    ...
    for (Luc_Count = FltBankStart; (Luc_Count <= FltBankEnd) && (Lbl_Success == FALSE); Luc_Count++)
    {
        ...
        {
            /* filter scale: 11bit or 29bit */
            CLEAR_BIT32(HwCanCtrlr[Idx].Filter->ulFS1Reg, Luc_Count);
            HwCanCtrlr[Idx].Filter->ulFS1Reg |= ((FltBankType >> 2) & 0x01) << Luc_Count;

            /* filter FIFO assignment */
            CLEAR_BIT32(HwCanCtrlr[Idx].Filter->ulFFA1Reg, Luc_Count);
            HwCanCtrlr[Idx].Filter->ulFFA1Reg |= (FltBankType & 0x01) << Luc_Count;

            /* set filter mode: CLEAR Identifier Mask or SET Identifier List */
            CLEAR_BIT32(HwCanCtrlr[Idx].Filter->ulFM1Reg, Luc_Count);
            HwCanCtrlr[Idx].Filter->ulFM1Reg |= ((FltBankType >> 1) & 0x01) << Luc_Count;
            ...
            /* activate filter */
            SET_BIT32(HwCanCtrlr[Idx].Filter->ulFA1Reg, Luc_Count);
...
{
    uint8 Luc_Idx;
    Can_ControllerConfigType *CtrlrPtr;
    CtrlrPtr = NULL_PTR;
    for (Luc_Idx = 0; Luc_Idx < ConfigPtr->ucNumCanController; Luc_Idx++)
    {
        if (ConfigPtr->stCanCtrlrs[Luc_Idx].ucId == CtrlrId)
        {
            CtrlrPtr = &ConfigPtr->stCanCtrlrs[Luc_Idx];
            break;
        }
        else
        {
    ...
    VAR(uint8, AUTOMATIC) BaudRateConfigID)
{
    ...
    for (Luc_Idx = 0; Luc_Idx < CtrlrPtr->ucBaudrateCfgSize; Luc_Idx++)
    {
